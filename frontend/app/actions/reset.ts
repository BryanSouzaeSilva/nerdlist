    ...
    try {
        ...
        await resend.emails.send({
            ...
            html: `
                ...
                <p style="color: #d4d4d4;">Você solicitou a recuperação de senha para a sua cofre no NerdList.</p>
        ...
        console.error("Erro fatal ao gerar ou enviar token de recuperação:", error);
        return { error: "O servidor de autenticação falhou ou o banco de dados está ocupado. Tente novamente." };
    }
}
...
export async function resetPassword(token: string, formData: FormData) {
    try {
        ...
        console.error("Erro fatal ao atualizar a nova senha:", error);
        return { error: "Não foi possível atualizar sua senha. O banco de dados recusou a conexão." };
    }
}