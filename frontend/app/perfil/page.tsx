...
import { FolderOpen, LayoutGrid, User, Star, ChevronRight } from "lucide-react";
...
function ProfileCard({ item }: { item: MediaItem }) {
    ...
    return (
        ...
        >
            <div className={`relative ${aspectClass} overflow-hidden`} style={{ position: 'relative' }}>
                {imageUrl ? (
                    <Image
                        src={imageUrl}
                        alt={item?.title || 'Item'}
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <header className="bg-linear-to-br from-neutral-900 to-neutral-800 p-8 rounded-3xl border border-neutral-700/50 mb-12 shadow-2xl relative overflow-hidden">
                    <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
                        ...
                        <div className="flex-1 text-center md:text-left">
                            <h1 className="text-3xl md:text-4xl font-black text-white mb-4">Bryan Silva</h1>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-center md:text-right">
                            <div className="bg-white/5 p-3 rounded-2xl border border-white/5 flex flex-col items-center">
                                <p className="text-2xl font-black text-white leading-none">{list.length}</p>
                ...
                <section className="mb-16">
                    ...
                    <div className="flex flex-wrap justify-center gap-4 md:gap-6 lg:gap-8">
                        {mediaTypes.map((type, index) => {
                            ...
                            return pinnedItem ? (
                                <div key={pinnedItem.id} className={`flex justify-center ${isLastItem ? 'col-span-2 md:col-span-1' : ''}`}>
                                    <ProfileCard item={pinnedItem} />
                                </div>
                            ) : (
                                <div key={type} className={`aspect-[2/3] rounded-xl border-2 border-dashed border-neutral-800 flex flex-col items-center justify-center p-4 text-center ${
                                    isLastItem ? 'col-span-2 md:col-span-1 w-[calc(50%-8px)] md:w-full mx-auto' : ''
                                }`}>
                                    <div className="w-10 h-10 rounded-full bg-neutral-900 flex items-center justify-center mb-3 text-neutral-700">
                                        <Star size={20} />
                                    </div>
                                    <p className="text-[9px] font-black text-neutral-600 uppercase tracking-tighter">Pinar {type}</p>
                                </div>
                            );
                        })}
                    </div>
                ...
                {list.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-neutral-500 border-2 border-dashed border-neutral-800 rounded-3xl">
                        ...
                        <Link href="/" className="mt-4 text-blue-400 hover:underline font-bold uppercase text-xs tracking-widest">Explorar catálogo</Link>
                    </div>
                ) : (
                    <div className="space-y-16">
                        {sections.map((section) => {
                            const filteredItems = list.filter(item => item.userListStatus === section.status);
