...
export default function MediaGrid({ items, userList = [] }: MediaGridProps) {
    return (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            {items.map((item) => {
                ...
                const savedInfo = userList.find(
                    (saved) => String(saved.mediaId) === String(item.id) && saved.type === item.type
                );
                ...
                return (
                    <Link
                        ...
                        className="group relative self-start bg-neutral-900 rounded-xl overflow-hidden hover:scale-105 transition-all duration-300 border border-neutral-800 shadow-lg block hover:z-10"
                    >
